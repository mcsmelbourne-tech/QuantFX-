...
TG_CONFIG_PATH = get_safe_path(".qfx_telegram_config.json")
SETTINGS_PATH = get_safe_path(".qfx_settings.json")
CONVICTION_STATE_PATH = get_safe_path(".qfx_conviction_state.json")
...
def evaluate_conviction_clause(df):
    ...
    passed = bool(
        c > e200
        and e9 > e200
        and c > float(high.iloc[-6])
        and c > float(high.iloc[-11])
        and e9 > e20
        and rsi_now > 50
        and lo <= e9
        and c > e9
...
with st.sidebar.expander("⚙️ MACD Settings", expanded=False):
    ...
    if st.button("💾 Save MACD settings", use_container_width=True, key="save_macd_settings"):
        ...
        st.success("Saved.") if ok else st.error("Could not write settings file.")
...
def run_scan_and_send(tg_token, tg_chat, ema_fast, ema_mid, ema_slow):
    ...
    if ok:
        save_conviction_state({"last": new_last, "ema30": active_ema30})
    return ok, total_alerts, ("Success" if ok else m)
...
with st.sidebar.expander("🔔 Telegram Alerts & Automated Triggers", expanded=False):
    ...
    if st.button("♻️ Reset alert memory", use_container_width=True):
...
if not AUTOREFRESH_AVAILABLE:
    ...
    st.sidebar.warning("Auto-scan is waiting for your Telegram Bot Token / Chat ID.")
else:
