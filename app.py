...
full share price display, 85% reduced chart box scaling, font size 10, right-side top mover cards (font size 11),
and Chartink Screener Share Export functionality.
"""
import json
import os
import re
...
def detect_ema_cross_signal(close_series, fast=21, slow=50, lookback=1):
  ...
  for i in range(n - 1, earliest - 1, -1):
    ...
    if f_now > s_now and f_prev <= s_prev:
      return {"direction": "BUY", "bars_ago": n - 1 - i, "fast": float(f_now), "slow": float(s_now)}
    if f_now < s_now and f_prev >= s_prev:
      return {"direction": "SELL", "bars_ago": n - 1 - i, "fast": float(f_now), "slow": float(s_now)}
  return None
...
def detect_market_structure(high, low, close, swing_lookback=5, brick_type=None):
  ...
  for i in range(n):
    ...
    broke_up = (pending_high is not None and pending_high_idx is not None and i > pending_high_idx and c > pending_high)
    broke_down = (pending_low is not None and pending_low_idx is not None and i > pending_low_idx and c < pending_low)
...
# =====================================================================
# CHARTINK EXPORT SHARES FUNCTION
# =====================================================================
def export_chartink_shares_to_csv(chartink_hits):
  """Converts Chartink screener hits into a CSV string format for download."""
...
def compute_7day_outlook(symbol, display, period="1y", interval="1d"):
  try:
    ...
    if last_hist > 0:
      bias_score += 0.5
      reasons.append("MACD Histogram is positive, favoring continuation of upward price impulses over the 7-day window.")
    else:
      bias_score -= 0.5
      reasons.append("MACD Histogram is negative, favoring downward continuation or corrective pullbacks.")
...
# =====================================================================
nifty200_raw = [
    "ABB", "ABFRL", "ACC", "ADANIENSOL", "ADANIENT", "ADANIGREEN", "ADANIPORTS", "ADANIPOWER", "AFFLE", "ALKEM", "AMBER", "APLAPOLLO", "APOLLOHOSP", "APOLLOTYRE", "ASIANPAINT", "ASTRAL", "AUBANK", "AXISBANK", "BAJAJFINSV", "BAJAJHFL", "BAJAJHLDNG", "BAJAJ_AUTO", "BAJFINANCE", "BANDHANBNK", "BANKBARODA", "BANKINDIA", "BATAINDIA", "BERGEPAINT", "BHARATFORG", "BHARTIARTL", "BHEL", "BIOCON", "BOSCHLTD", "BPCL", "BRITANNIA", "BSE", "CANBK", "CANFINHOME", "CDSL", "CEATLTD", "CGPOWER", "CHOLAFIN", "CIPLA", "CNX200", "COALINDIA", "COCHINSHIP", "COFORGE", "COLPAL", "CONCOR", "COROMANDEL", "CUMMINSIND", "DALBHARAT", "DEEPAKNTR", "DIVISLAB", "DIXON", "DLF", "DMART", "DRREDDY", "EICHERMOT", "ESCORTS", "EVEREADY", "EXIDEIND", "FACT", "FEDERALBNK", "FLUOROCHEM", "FORTIS", "GLENMARK", "GODREJCP", "GODREJPROP", "GOLDBEES", "GRASIM", "HAL", "HAVELLS", "HCLTECH", "HDFCAMC", "HDFCBANK", "HDFCGOLD", "HDFCLIFE", "HDFCSILVER", "HEROMOTOCO", "HINDALCO", "HINDPETRO", "ICICIBANK", "ICICIGI", "ICICIPRULI", "IDFCFIRSTB", "IEX", "IGL", "INDHOTEL", "INDIANB", "INDIGO", "INDUSINDBK", "INDUSTOWER", "INFY", "IOC", "IPCALAB", "IRCTC", "IREDA", "IRFC", "ITC", "JINDALSTEL", "JIOFIN", "JKCEMENT", "JSWENERGY", "JSWSTEEL", "JUBLFOOD", "KOTAKBANK", "KPITTECH", "LALPATHLAB", "LAURUSLABS", "LICHSGFIN", "LICI", "LINDEINDIA", "LODHA", "LT", "LTIM", "LTTS", "LUPIN", "M&M", "M&MFIN", "MARICO", "MARUTI", "MAXHEALTH", "MAZDOCK", "MCX", "MFSL", "MOTHERSON", "MPHASIS", "MRF", "MSUMI", "MUTHOOTFIN", "NATIONALUM", "NAUKRI", "NAVINFLUOR", "NELCO", "NESTLEIND", "NHPC", "NIFTY", "NMDC", "NTPC", "OBEROIRLTY", "OIL", "ONGC", "PAGEIND", "PATANJALI", "PAYTM", "PERSISTENT", "PETRONET", "PFC", "PGHH", "PIDILITIND", "PIIND", "PNB", "PNBHOUSING", "POLICYBZR", "POLYCAB", "POONAWALLA", "POWERGRID", "POWERINDIA", "PRESTIGE", "RAILTEL", "RAMCOCEM", "RECLTD", "RELIANCE", "ROUTE", "RVNL", "SAIL", "SBICARD", "SBILIFE", "SBIN", "SHREECEM", "SHRIRAMFIN", "SIEMENS", "SONACOMS", "SRF", "SUNPHARMA", "SUNTV", "SYNGENE", "TANLA", "TATACHEM", "TATACOMM", "TATACONSUM", "TATAELXSI", "TATAPOWER", "TATASTEEL", "TATATECH", "TCS", "TECHM", "TEJASNET", "TIINDIA", "TITAN", "TMPV", "TORNTPHARM", "TORNTPOWER", "TRENT", "TVSMOTOR", "UBL", "ULTRACEMCO", "UNITDSPR", "VBL", "VEDL", "VOLTAS", "HINDCOPPER", "NDIA"
]
us100_raw = [
    "PLTR", "ARM", "INTC", "AMD", "MU", "QCOM", "LRCX", "MCHP", "AVGO", "AMAT", "GFS", "TXN", "IDXX", "DDOG", "ZS", "TRI", "CSCO", "ADI", "PANW", "ORCL", "AXON", "CRWD", "ASML", "SLV", "CHTR", "TTD", "SHOP", "NAS100", "APP", "BIIB", "FUTU", "PCAR", "NVDA", "FTNT", "MSFT", "FAST", "VRTX", "US30", "WDAY", "CDNS", "SPX", "ORLY", "ON", "CSX", "TSLA", "AAPL", "SBUX", "GLD", "ADBE", "PDD", "LIN", "BKR", "GOOGL", "HON", "PYPL", "INTU", "ADSK", "CMCSA", "DASH", "ROST", "GILD", "KHC", "CTAS", "AEP", "EA", "DXCM", "XEL", "GEHC", "BKNG", "MDLZ", "EXC", "WBD", "MNST", "LULU", "TMUS", "PEP", "ADP", "NFLX", "ABNB", "COST", "CTSH", "MELI", "TTWO", "META", "CSGP", "CEG", "AMZN", "ISRG", "CCEP", "FANG"
]
...
FOREX_PAIRS = [("EURUSD=X", "EUR/USD"), ("GBPUSD=X", "GBP/USD"), ("USDJPY=X", "USD/JPY"), ("AUDUSD=X", "AUD/USD"), ("USDCAD=X", "USD/CAD")]
...
TIMEFRAME_PERIODS = {"15m": "10d", "30m": "20d", "60m": "60d", "4h": "180d", "1d": "1y", "1wk": "5y"}
...
def create_chart_figure(renko_df, ha_df, brick_size, display, ema_fast, ema_slow):
  ...
  fig.add_trace(go.Scatter(x=x_ha, y=ha_df["EMA_FAST"], line=dict(color=COLOR_MA_FAST, width=1.5), name=f"HA EMA {ema_fast}", showlegend=False), row=1, col=1)
  fig.add_trace(go.Scatter(x=x_ha, y=ha_df["EMA_SLOW"], line=dict(color=COLOR_MA_SLOW, width=1.5), name=f"HA EMA {ema_slow}", showlegend=False), row=1, col=1)
  ...
  fig.add_trace(go.Bar(x=x_renko, y=hist_vals, marker_color=hist_colors, name="MACD Histogram", opacity=0.8, showlegend=False), row=3, col=1)
  fig.add_trace(go.Scatter(x=x_renko, y=renko_df["MACD"], line=dict(color=COLOR_MACD_LINE, width=1.8), name="MACD Line", showlegend=False), row=3, col=1)
  fig.add_trace(go.Scatter(x=x_renko, y=renko_df["MACD_Signal"], line=dict(color=COLOR_SIGNAL_LINE, width=1.8), name="Signal Line", showlegend=False), row=3, col=1)
  ...
  macd_pad = ((macd_finite.max() - macd_finite.min()) * 0.06 or abs(macd_finite.max()) * 0.06 or 0.001) if macd_finite.size else 0.001
...
def render_clickable_single_box(title, movers, key_prefix, on_click):
  ...
  _render_clickable_html(
      marker,
      inner,
      extra_style=f"background-color:{COLOR_PANEL_BG};border:1px solid {COLOR_BORDER};border-radius:6px;padding:8px 12px;margin-bottom:8px;",
...
def render_clickable_list_box(title, movers, key_prefix, on_click, value_fmt=None):
  ...
  for idx, m in enumerate(movers, start=1):
    ...
    inner = f"<div style='font-size:11px;display:flex;justify-content:space-between;gap:6px;color:{COLOR_TEXT_MAIN};'><span>{idx}. {m['display']}</span>{value_html}</div>"
  ...
  if raw_df.empty:
    ...
    if renko_df.empty:
      ...
      with chart_col:
        ...
        badge_color = (
            COLOR_GREEN if last_confirmed == "BUY" else (COLOR_RED if last_confirmed == "SELL" else COLOR_TEXT_MUTED)
        )
        st.markdown(
      ...
      with right_panel_col:
        ...
        # CHARTINK-EMA9&21 CROSS BOX with Export functionality
        render_clickable_list_box(
            "📊 CHARTINK-EMA9&21 CROSS BOX",
            chartink_hits,
            key_prefix="chartink_ema920",
            on_click=go_to_chart,
        )
        
        # Export Shares button/feature for Chartink
        csv_export_data = export_chartink_shares_to_csv(chartink_hits)
        if csv_export_data:
          st.download_button(
              label="📥 Export Chartink Shares (CSV)",
              data=csv_export_data,
              file_name="chartink_ema9_20_cross_shares.csv",
              mime="text/csv",
              key="export_chartink_csv",
              use_container_width=True,
          )
        ...
        st.markdown(
            ...
            f" style='color:{COLOR_TEXT_MUTED};text-decoration:none;'>Open full screener on Chartink ↗</a>"
            f"</div>",
            unsafe_allow_html=True,
        )
        if outlook:
          dir_color = (
              COLOR_GREEN if outlook["direction"] == "Bullish" else (COLOR_RED if outlook["direction"] == "Bearish" else COLOR_TEXT_MUTED)
          )
          reasons_html = "".join(
        ...
        render_clickable_single_box("Top Commodity", top_commodity, key_prefix="open_top_commodity", on_click=go_to_chart)
        render_clickable_single_box("Top Forex", top_forex, key_prefix="open_top_forex", on_click=go_to_chart)
