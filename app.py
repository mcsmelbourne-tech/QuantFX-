...
v11 additions (page-load fixes + background alerts):
- The page no longer freezes while ~600 symbols are scanned: the chart is drawn FIRST, the right-hand
  scanner boxes load after it (cached 15 min), the Chartink box loads last, and the in-app auto-scan is opt-in.
- The chart iframe uses st.iframe (st.components.v1.html is being removed from Streamlit) and loads a plotly.js
  build that matches your installed plotly, with CDN fallbacks and a visible message instead of a blank box.
- All scan / alert logic now lives in qfx_core.py, so scan_job.py (GitHub Actions or any server) sends the exact
...
import qfx_core
from qfx_core import (
    COMMODITIES, FOREX_PAIRS, CONVICTION_UNIVERSE,
    nifty500_raw, nifty500_yf, us100_raw, us100_yf,
    format_price, conviction_price_str,
    load_telegram_config, save_telegram_config,
    send_telegram_alert, save_conviction_state,
)

# Streamlit is replacing **STRETCH with width="stretch"; support both.
STRETCH = (
    {"width": "stretch"}
...
st.markdown(
    ...
    div[class*="st-key-qfxrow_"] button div[data-testid="stMarkdownContainer"] {{
...
# =====================================================================
# TELEGRAM CONFIGURATION  ->  moved to qfx_core.py (shared with scan_job.py)
# =====================================================================
...
def fetch_live_ohlc(symbol="GC=F", period="6mo", interval="1d"):
  return qfx_core.fetch_ohlc(symbol, period=period, interval=interval)
...
# WATCHLISTS
# =====================================================================
# The Nifty 500 / US 100 / Commodities / Forex lists now live in qfx_core.py (imported above).
...
def _brick_positions(ha_dates, renko_df, n_ha):
  """x position of every Renko brick on the real-candle axis (candle index it formed on; several bricks
  formed in one candle are spread inside that candle's slot). Lets Renko + RSI line up with Heikin Ashi."""
  n_rk = len(renko_df)
  pos = None
  try:
    if ha_dates and "Date" in renko_df.columns:
      ...
      if c_dates.is_monotonic_increasing:
        ...
        pos = c_idx - 0.5 + (k_in + 0.5) / m_in
  except Exception:
    pos = None
  if pos is None or not np.all(np.diff(pos) > 0):
    pos = np.linspace(0, max(n_ha - 1, 1), n_rk)
...
"""
_CHART_TEMPLATE = r"""
<div id="__ID___wrapper" style="
    resize: both; overflow: auto; width: 100%; height: __HEIGHT__px;
    min-width: 320px; min-height: 400px; max-width: 100%;
    border: 1px solid __BORDER__; border-radius: 6px;
    background-color: __BG__; padding: 4px; box-sizing: border-box;">
...
(function() {
  ...
  var wrapper = document.getElementById("__ID___wrapper");
  ...
  function draw() {
    ...
    if (window.ResizeObserver) {
      new ResizeObserver(function() { Plotly.Plots.resize(el); }).observe(wrapper);
    }
  }

  function fail() {
    el.innerHTML = '<div style="color:#FF4F7B;font:13px sans-serif;padding:16px;line-height:1.6">' +
      'Could not download the Plotly chart library (tried ' + urls.length + ' sources).<br>' +
      'Check your internet / firewall / ad-blocker for cdn.plot.ly and cdn.jsdelivr.net, then press R.</div>';
  }
  ...
  function tryLoad(i) {
    ...
    s.src = urls[i];
    s.onload = function() { if (window.Plotly) { draw(); } else { tryLoad(i + 1); } };
...
def _plotly_js_urls():
  """plotly.js build that matches the installed plotly.py (plotly.py 6/7 write base64 typed arrays that
  older plotly.js builds cannot read), then CDN mirrors, then a known-good older build."""
  try:
    ver = _plotly_offline.get_plotlyjs_version()
  except Exception:
    ver = "2.35.2"
  return [
      f"https://cdn.plot.ly/plotly-{ver}.min.js",
      f"https://cdn.jsdelivr.net/npm/plotly.js-dist-min@{ver}/plotly.min.js",
...
def render_zoomable_chart(fig, key, height=950):
  div_id = "qfx_chart_" + re.sub(r"\W", "_", str(key))
  html = (
      _CHART_TEMPLATE
      .replace("__HEIGHT__", str(int(height)))
      .replace("__BORDER__", COLOR_BORDER)
      .replace("__BG__", COLOR_BG_DARK)
      .replace("__MUTED__", COLOR_TEXT_MUTED)
      .replace("__SYNC_JS__", QFX_SYNC_JS)
      .replace("__URLS__", json.dumps(_plotly_js_urls()))
      .replace("__ID__", div_id)
      .replace("__FIG_JSON__", fig.to_json())   # last: the figure JSON must never be re-scanned
  )
...
def render_clickable_single_box(title, movers, key_prefix, on_click, compact=False):
  ...
  st.button(
      ...
      args=(best["symbol"], best["display"]), **STRETCH,
  )
...
def render_clickable_list_box(title, movers, key_prefix, on_click, value_fmt=None, empty_text="No data"):
  ...
  st.markdown(
      f"<div style='background-color:{COLOR_PANEL_BG};border:1px solid {COLOR_BORDER};"
      f"border-radius:6px 6px 0 0;padding:8px 12px 22px 12px;margin-bottom:0px;'>"
...
with st.sidebar.expander("⚙️ MACD Settings", expanded=False):
  ...
  if st.button("💾 Save MACD settings", **STRETCH, key="save_macd_settings"):
    ok = save_settings({
        **load_settings(),
...
with st.sidebar:
  _chartink_slot = st.container()   # filled at the very end of the script, so a slow Chartink never blocks the page
st.sidebar.markdown("---")
...
def run_scan_and_send(tg_token, tg_chat, ema_fast, ema_mid, ema_slow):
  """Same alert run the cloud job uses (qfx_core.run_scan_and_send), fed by this app's cached scanners."""
  return qfx_core.run_scan_and_send(
      tg_token, tg_chat, ema_fast, ema_mid, ema_slow,
      conviction_scan=scan_conviction_category,
      cross30_scan=_scan_triple_ema_cross_30m,
  )
...
with st.sidebar.expander("🔔 Telegram Alerts & Automated Triggers", expanded=False):
  ...
  st.caption(
      ...
      "To get these while your computer and phone are OFF, run the cloud job (scan_job.py — see SETUP.md). "
      "The cloud job keeps its own alert memory, so use either the cloud job or the in-app timer, not both."
  )
  if st.button("🚀 Run Auto Scan & Send", **STRETCH):
    with st.spinner("Scanning all watchlists (first run can take a couple of minutes)..."):
  ...
  if st.button("♻️ Reset alert memory (next scan sends full list)", **STRETCH):
  ...
  auto_scan_on = st.checkbox(
      "Also auto-scan while this browser tab is open", value=False,
      help="Off by default: the scan runs inside the page, so it only works while the tab is open. "
           "Use the cloud job for real background alerts.",
  )
  auto_scan_minutes = st.selectbox(
      "Auto-scan interval", [5, 15, 30, 60], index=1,
      format_func=lambda m: f"every {m} min", disabled=not auto_scan_on,
  )
...
if active_view == "📊 Charts":
  ...
  if raw_df.empty:
    ...
    st.caption("Yahoo Finance may be rate-limiting or the symbol may be wrong — wait a minute and press 🔄 Refresh data.")
  else:
    ...
    if renko_df.empty:
      ...
      with right_panel_col:
        def _ema_cross_value_md(cat_name):
          ...
          return _fmt
        with st.spinner("Loading 7-day outlook..."):
...
# =====================================================================
# DEFERRED BLOCKS — run last so they never delay the first paint
# =====================================================================
with _chartink_slot:
...
# Optional in-app timer (off by default). It only works while this tab is open - the cloud job is the real background alert.
with _auto_status_slot:
  if auto_scan_on:
    if not AUTOREFRESH_AVAILABLE:
      st.warning("The in-app timer needs `streamlit-autorefresh`:  pip install streamlit-autorefresh")
    elif not (tg_token and tg_chat):
      st.warning("Auto-scan is waiting for your Telegram Bot Token / Chat ID (open 🔔 Telegram Alerts and save them).")
    else:
      _timer_key = f"qfx_auto_refresh_timer_{int(auto_scan_minutes)}"
      _count_key = f"_qfx_last_count_{_timer_key}"
      _refresh_count = st_autorefresh(interval=int(auto_scan_minutes) * 60 * 1000, key=_timer_key)
      if _refresh_count != st.session_state.get(_count_key, -1):
        st.session_state[_count_key] = _refresh_count
